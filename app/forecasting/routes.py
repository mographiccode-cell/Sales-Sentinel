...
def _daily_sales_rows(db):
    """Prefer transaction-level history when it is available to avoid double counting seeded aggregates."""
    rich = db.execute(
        select(Sale.sale_date, func.sum(Sale.net_sales))
        .where(Sale.transaction_type.notin_(["DAILY_AGGREGATE", "DAILY_IMPORT"]))
    ...
    return rows, "aggregate_fallback"
            ...
            with session_scope() as db:
                ...
                if decline_risk.get("available"):
                    ...
                    metrics = {
                        ...
                        "point_forecast_engine": {"name": point_model_name, "version": point_model_version, "metrics": generated[0]["metrics"]},
                    }
                else:
                ...
                run = ModelRun(
                    ...
                    status="completed", horizon_days=horizon,
                    filters_json={"data_mode": data_mode, "decline_engine_available": bool(decline_risk.get("available"))},
                    metrics_json=metrics, data_start=rows[0][0], data_end=rows[-1][0],
                ...
                for item in generated:
                    ...
                    db.add(forecast_row); db.flush()
                    highest = forecast_row if highest is None or forecast_row.decline_probability > highest.decline_probability else highest

                should_alert = bool(decline_risk.get("alert")) if decline_risk.get("available") else bool(highest and highest.decline_probability >= 0.55)
                if highest and should_alert:
                    ...
                    if decline_risk.get("available"):
                        ...
                        alert = Alert(
                            ...
                            title_ar="إنذار مبكر لاحتمال انخفاض المبيعات", title_en="Early sales-decline warning",
                        ...
                        alert = Alert(
                            forecast_id=highest.id, severity=severity,
                            title_ar="احتمال انخفاض مبيعات - وضع مبسط", title_en="Sales decline probability - minimal mode",
                    ...
                    db.add(DeclineFactor(
                        ...
                        impact_value=highest.decline_percent, direction="negative", method=factor_method,
                    ))
                    db.add(Recommendation(
                        ...
                        rationale_en="This is decision support, not an automated action; the RED/critical channel remains disabled pending sufficient evidence.", priority=1,
                    ))
                run_id = run.id
