...
ROOT=Path(__file__).resolve().parents[1]; DATA=ROOT/'data'/'sama_pos'/'sama_city_weekly_value_count_2020_2026_extended.csv'; OUT=ROOT/'reports'/'sama_city_v3_3'; OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ...
 for origin in sorted(d.loc[d.week_start.between(START,END),'week_start'].unique()):
  ...
  for p in r['predictions']:rows.append({'week_start':pd.Timestamp(p['week_start']),'city':p['city'],'state':p['state'],'reason':p['reason'],'risk_score':p['risk_score'],'ood_fraction':p['ood_fraction'],'precursor_count':p['precursor_count']})
