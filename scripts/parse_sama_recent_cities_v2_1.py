...
# Historical KAPSARC labels -> English labels observed in the official post-Jul-2025 SAMA PDF city table.
# Aliases are explicit; no fuzzy geographic inference is allowed.
CITY_ALIASES={
    'ABHA':['Abha'],
...
def match_city(line:str,cities:set[str]):
    ...
    for canonical,aliases in CITY_ALIASES.items():
        ...
        for alias in aliases:
            a=common.norm(alias)
            # The English city label is at the beginning of each Table 2.1 data row.
...
def parse_city_pdf(path:Path,cities:list[str]):
    ...
    for line in lines:
        ...
        # Unlike Table 1, Table 2.1 places city label and all numeric cells ON THE SAME LINE.
        vals=[float(x.replace(',','')) for x in common.NUM_RE.findall(line)]
        if len(vals)<8:
            continue
        # First 8 numbers are four (transaction count, transaction value) pairs.
        for k,(ws,we) in enumerate(ranges):
            rows.append({
                ...
                'source_pdf':path.name,'source_city_label':line.strip(),
                'source_city_alias':alias,'report_latest_week_end':ranges[-1][1].normalize(),
            })
        found[city]={'alias':alias,'line':line.strip()}
    missing=sorted(city_set-set(found))
    if missing:
        raise RuntimeError(f'{path.name}: missing historical city labels after explicit alias mapping: {missing}')
    return rows,{
        'file':path.name,
...
def main():
    ...
    if missing_alias_defs:
        raise RuntimeError(f'No explicit fresh-PDF alias definition for historical cities: {missing_alias_defs}')
    ...
    spread=hraw.groupby(['week_start','city']).agg(
        n=('value_thousand_sar','size'),value_min=('value_thousand_sar','min'),value_max=('value_thousand_sar','max'),
    ...
    # Overlapping four-week official reports are deduplicated by the latest report after consistency is measured.
    d=(hraw.sort_values(['week_start','city','report_latest_week_end'])
         .drop_duplicates(['week_start','city'],keep='last')
         .sort_values(['city','week_start']).reset_index(drop=True))
    ...
    city_week_sets={c:tuple(q.week_start.sort_values().tolist()) for c,q in d.groupby('city')}
    ref=next(iter(city_week_sets.values())) if city_week_sets else tuple()
    all_city_week_keys_identical=all(v==ref for v in city_week_sets.values())

    # Reconcile fresh 11-city total with fresh national total parsed independently from the same PDFs when available.
    national_path=ROOT/'data'/'sama_pos'/'sama_national_weekly_value_count_2025_2026_holdout.csv'
    reconciliation=None
    reconciliation_checks=True
    if national_path.exists():
        nat=pd.read_csv(national_path,parse_dates=['week_start'])
        ...
        if len(rr):
            rr['value_rel_diff']=(rr.city_value-rr.value_thousand_sar).abs()/rr.value_thousand_sar
            rr['count_rel_diff']=(rr.city_count-rr.transaction_count).abs()/rr.transaction_count
            reconciliation={
                'weeks_compared':int(len(rr)),
                'max_value_relative_difference':float(rr.value_rel_diff.max()),
                'max_count_relative_difference':float(rr.count_rel_diff.max()),
            }
    ...
    checks={
        ...
        'positive_counts':bool((d.transaction_count_thousand>0).all()),
    ...
    audit={
        'version':'SAMA-CITY-FRESH-HOLDOUT-2.1.1',
        'scientific_boundary':'Official SAMA City Total weekly POS only; independent of sector taxonomy revision. City row parsing follows the observed same-line Table 2.1 layout with explicit aliases.',
        'pdfs':len(paths),'parsed_pdfs':len(files),'failures':failures,
        'rows':int(len(d)),'weeks':int(d.week_start.nunique()),'cities':cities,'city_count':len(cities),
        'date_start':str(d.week_start.min().date()),'date_end':str(d.week_start.max().date()),
    ...
    if not audit['all_checks_passed']:
        raise RuntimeError('Fresh SAMA city holdout quality gate failed')
