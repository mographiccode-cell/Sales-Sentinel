        ...
        for customer_index in range(customer_count):
            ...
            sheet.append(
                [
                    ...
                    f"TRX-{day_index:03d}-{customer_index}",
                    channel,
                    f"CUST-{customer_index:03d}",
                    f"ITEM-{customer_index % 5:02d}",
                    "Retail",
                    "General",
                    f"Sub-{customer_index % 3}",
                    "Saudi Demo",
                    "INV",
                    quantity,
                    unit_price,
                    discount,
                    4 if discount else 0,
                    net,
                    vat,
                    total,
                ]
...
def bypass_submit(page: Page, form_selector: str) -> None:
    page.locator(form_selector).evaluate(
        "form => { form.noValidate = true; form.submit(); }"
    )
...
def capture_browser_evidence() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        page.fill(
            '#new-user-form input[name="password_confirmation"]',
            "DifferentPass123!",
        )
        bypass_submit(page, "#new-user-form form")
        open_user_form(page)
        ...
        page.locator('#sales-filters input[name="start"]').evaluate(
            "el => { el.type = 'text'; el.value = 'not-a-date'; }"
        )
        ...
        page.set_input_files(
            'form.ux3-upload-form input[name="file"]', str(unsupported_path)
        )
        ...
        run_href = page.locator("tr[data-row-href]").first.get_attribute(
            "data-row-href"
        )
        if not run_href:
            raise RuntimeError(
                "No analysis detail route was created after real browser upload"
            )
        ...
        page.fill(
            '#new-user-form input[name="email"]', "reviewanalyst@example.com"
        )
        ...
        screenshot(page, INTERFACE / "12- System health screen.png")
        page.goto(f"{BASE_URL}/admin/settings", wait_until="networkidle")
        screenshot(page, INTERFACE / "13- Settings interface.png")
...
body{{display:inline-block;min-width:1180px;max-width:1400px}}
.frame{{display:inline-block;min-width:1180px;max-width:1400px;background:#1e1e1e;padding:18px 22px 18px 12px}}
.highlighttable{{border-spacing:0;border-collapse:collapse;margin:0;width:100%}}
.linenos{{width:64px;min-width:64px;padding:0 16px 0 4px!important;border-right:1px solid #343434;vertical-align:top;user-select:none}}
.linenos pre{{color:#858585!important;text-align:right!important;background:transparent!important}}
.code{{padding-left:18px!important;vertical-align:top;width:100%}}
pre{{margin:0!important;font-family:'Cascadia Code','DejaVu Sans Mono','Liberation Mono',monospace!important;font-size:15px!important;line-height:1.48!important;tab-size:4;white-space:pre-wrap!important;overflow-wrap:normal;word-break:normal;direction:ltr;text-align:left}}
</style></head><body><div class="frame">{rendered}</div></body></html>"""
...
def capture_code_evidence() -> None:
    functions = [
        ...
        (
            "6- Instant analysis function.png",
            "app/services/instant_analysis.py",
            "run_instant_analysis",
            42,
        ),
        (
            "7- Decline explanation function.png",
            "app/services/decline_explainer.py",
            "explain_decline_drivers",
            42,
        ),
        (
            "8- Report generation function.png",
    ...
    error_blocks = [
        (
            "1- Username or password test code.png",
            "app/auth/routes.py",
        ...
        (
            "3- Password confirmation test code.png",
            "app/admin/routes.py",
        ...
        (
            "4- Missing required field test code.png",
        ...
        (
            "6- Unsupported file test code.png",
            "app/imports/routes.py",
...
def verify_output() -> None:
    expected_directories = {
        "Interface screens",
        "Error screens",
        "Code function screenshots",
        "Code error screenshots",
    }
    actual_directories = {path.name for path in SCREENSHOTS.iterdir() if path.is_dir()}
    if actual_directories != expected_directories:
        raise AssertionError(
            f"Unexpected screenshot directories: {sorted(actual_directories)}"
        )
    ...
    for folder, expected_count in expected_counts.items():
        ...
        if len(files) != expected_count:
            raise AssertionError(
                f"{folder.name} has {len(files)} PNG files; expected {expected_count}"
            )
        for file in files:
            ...
            if height > 1000:
                raise AssertionError(
                    f"Image exceeds 1000px height: {file} -> {height}"
                )
            manifest.append(
                {
                    "name": file.name,
    ...
    error_code_names = {
        path.name.replace(" code.png", "") for path in ERROR_CODE.glob("*.png")
    }
    if error_names != error_code_names:
        raise AssertionError(
            "Error screen names and error-code screenshot names do not match: "
            f"screens={sorted(error_names)} code={sorted(error_code_names)}"
        )
