...
MODEL=ROOT/'models'/'sama_city_v3_4'/'city_risk_v3_4.joblib'
ENGINE_VERSION='SALES-SENTINEL-CITY-RISK-ENGINE-3.4'
...
def predict_latest(panel:pd.DataFrame,model_path:Path=MODEL)->dict[str,Any]:
    if not model_path.exists(): raise RuntimeError(f'v3.4 model missing: {model_path}')
    ...
    core_cols=a['structural_core_columns']; core_count=pp[core_cols].sum(axis=1).astype(int)
    out=[]
    for i,row in mm.iterrows():
        sc=float(risk[i]); pci=int(pcount.iloc[i]); cci=int(core_count.iloc[i]); ood=ood_fraction(xx.iloc[i],a['ood_profile']); precursor_names=[c for c in pp.columns if bool(pp.iloc[i][c])]
        red=sc>=float(a['red_threshold']) and int(agreement[i])>=2 and pci>=int(a['min_precursor_red'])
        primary=sc>=float(a['watch_threshold'])
        structural=cci>=int(a['structural_core_min']) and sc>=float(a['structural_trend_score_floor'])
        ...
        elif structural: state='AMBER'; reason='STRUCTURAL_TREND_WARNING'
        else: state='GREEN'; reason='LOW_OBSERVED_RISK'
        out.append({'week_start':str(pd.Timestamp(row.week_start).date()),'city':row.city,'risk_score':sc,'model_scores':{n:float(ms[n][i]) for n in names},'model_agreement_ge_0_5':int(agreement[i]),'precursor_count':pci,'structural_core_count':cci,'precursors':precursor_names,'ood_fraction':ood,'state':state,'reason':reason})
    return {'engine_version':ENGINE_VERSION,'model_version':a['version'],'status':'OK','latest_week':str(latest.date()),'predictions':out,'safety':{'future_label_used':False,'city_identity_feature_used':False,'target_prevalence_feature_used':False,'absolute_level_feature_used':False,'structural_channel_current_past_only':True,'ood_fail_closed_to_amber':True,'surprise_shocks_claimed_predictable':False}}
