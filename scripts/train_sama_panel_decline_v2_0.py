...
def prepare():
    ...
    # Avoid duplicated aggregate hierarchies; model only concrete city-sector panels.
    d=d[~d.city.astype(str).str.strip().str.lower().eq('total') & ~d.sector.astype(str).str.strip().str.lower().eq('total')].copy()
    d['pos_value_thousand_sar']=pd.to_numeric(d['pos_value_thousand_sar'],errors='coerce'); d['transaction_count']=pd.to_numeric(d['transaction_count'],errors='coerce'); d=d.dropna(); d=d[(d.pos_value_thousand_sar>0)&(d.transaction_count>0)].sort_values(['city','sector','week_start'])
    # Only entities with enough history and reasonably consecutive observations.
    counts=d.groupby(['city','sector']).size(); keep=set(counts[counts>=80].index); d=d[d.set_index(['city','sector']).index.isin(keep)].copy()
    g=d.groupby(['city','sector'],sort=False)
    d['week_gap_days']=g.week_start.diff().dt.days
    # Continuous features based only on information known at origin week.
    for col,prefix in [('pos_value_thousand_sar','value'),('transaction_count','count')]:
        ...
        for lag in (1,2,3,4,8,13,26,52): d[f'log_{prefix}_lag_{lag}']=g[col].shift(lag).pipe(np.log1p)
    ...
    # National totals, lagged/current at the same origin week, are known and useful market context.
    nat_v=sub(vm,'national_value').query("city.str.lower() == 'total' and sector.str.lower() == 'total'",engine='python')[['week_start','national_value']]
    nat_n=sub(nm,'national_count').query("city.str.lower() == 'total' and sector.str.lower() == 'total'",engine='python')[['week_start','national_count']]
    nat=nat_v.merge(nat_n,on='week_start',how='inner').sort_values('week_start')
    for col in ('national_value','national_count'):
        nat[f'{col}_pct1']=nat[col].pct_change(1); nat[f'{col}_pct4']=nat[col].pct_change(4); nat[f'{col}_mean13']=nat[col].rolling(13).mean(); nat[f'{col}_ratio13']=nat[col]/nat[f'{col}_mean13']
    d=d.merge(nat,on='week_start',how='left')
    ...
    # Target uses next week value versus trailing four-week mean known at origin.
    d['baseline4']=g.pos_value_thousand_sar.transform(lambda x:x.rolling(4).mean()); d['next_value']=g.pos_value_thousand_sar.shift(-1); d['next_ratio']=d.next_value/d.baseline4.replace(0,np.nan)
    # Require current and next records to be consecutive calendar weeks.
    d['next_week_start']=g.week_start.shift(-1); d['next_gap_days']=(d.next_week_start-d.week_start).dt.days
    cat=pd.get_dummies(d[['city','sector']],prefix=['city','sector'],dtype=float); d=pd.concat([d,cat],axis=1)
    exclude={'week_start','next_week_start','city','sector','pos_value_thousand_sar','transaction_count','next_value','next_ratio'}
    features=[c for c in d.columns if c not in exclude and not c.startswith('target_')]
    # week_gap_days is provenance only and its first observation is null; keep only proper sequences.
