...
def models(): return {
    'LogisticRegression':Pipeline([('scale',StandardScaler()),('model',LogisticRegression(C=.5,max_iter=5000,class_weight='balanced',random_state=SEED))]),
    'RandomForest':RandomForestClassifier(n_estimators=900,max_depth=12,min_samples_leaf=3,max_features=.65,class_weight='balanced_subsample',random_state=SEED,n_jobs=-1),
    'ExtraTrees':ExtraTreesClassifier(n_estimators=1200,max_depth=14,min_samples_leaf=2,max_features=.70,class_weight='balanced',random_state=SEED,n_jobs=-1),
    'HistGradientBoosting':HistGradientBoostingClassifier(learning_rate=.035,max_iter=450,max_leaf_nodes=24,min_samples_leaf=20,l2_regularization=2,random_state=SEED)
}
...
def prepare():
    ...
    def sub(mask,name):
        z=raw.loc[mask,['week_start','city','sector','value']].copy(); z['value']=pd.to_numeric(z['value'],errors='coerce')
        return z.rename(columns={'value':name}).drop_duplicates(['week_start','city','sector'],keep='last')
    v=sub(vm,'pos_value_thousand_sar'); n=sub(nm,'transaction_count')
    d=v.merge(n,on=['week_start','city','sector'],how='inner')
    city_total=d.city.astype(str).str.strip().str.lower().ne('total') & d.sector.astype(str).str.strip().str.lower().eq('total')
    sector_total=d.city.astype(str).str.strip().str.lower().eq('total') & d.sector.astype(str).str.strip().str.lower().ne('total')
    d=d[city_total|sector_total].copy()
    d['entity_type']=np.where(d.sector.astype(str).str.strip().str.lower().eq('total'),'City','Sector')
    d['entity_name']=np.where(d['entity_type'].eq('City'),d.city.astype(str),d.sector.astype(str))
    d['entity_id']=d.entity_type+':'+d.entity_name
    d=d.dropna(subset=['pos_value_thousand_sar','transaction_count']); d=d[(d.pos_value_thousand_sar>0)&(d.transaction_count>0)].sort_values(['entity_id','week_start']).reset_index(drop=True)
    counts=d.groupby('entity_id').size(); keep=set(counts[counts>=80].index); d=d[d.entity_id.isin(keep)].sort_values(['entity_id','week_start']).reset_index(drop=True)
    g=d.groupby('entity_id',sort=False); d['week_gap_days']=g.week_start.diff().dt.days
    ...
    # Official national total context is known at the origin week and is not a future feature.
    nat_v=v[v.city.astype(str).str.strip().str.lower().eq('total') & v.sector.astype(str).str.strip().str.lower().eq('total')][['week_start','pos_value_thousand_sar']].rename(columns={'pos_value_thousand_sar':'national_value'})
    nat_n=n[n.city.astype(str).str.strip().str.lower().eq('total') & n.sector.astype(str).str.strip().str.lower().eq('total')][['week_start','transaction_count']].rename(columns={'transaction_count':'national_count'})
    nat=nat_v.merge(nat_n,on='week_start',how='inner').sort_values('week_start').drop_duplicates('week_start').reset_index(drop=True)
    ...
    d=d.merge(nat,on='week_start',how='left').sort_values(['entity_id','week_start']).reset_index(drop=True)
    ...
    g2=d.groupby('entity_id',sort=False); d['baseline4']=g2.pos_value_thousand_sar.transform(lambda x:x.rolling(4).mean()); d['next_value']=g2.pos_value_thousand_sar.shift(-1); d['next_ratio']=d.next_value/d.baseline4.replace(0,np.nan); d['next_week_start']=g2.week_start.shift(-1); d['next_gap_days']=(d.next_week_start-d.week_start).dt.days
    cat=pd.get_dummies(d[['entity_type','entity_name']],prefix=['type','entity'],dtype=float); d=pd.concat([d,cat],axis=1)
    exclude={'week_start','next_week_start','next_gap_days','city','sector','entity_type','entity_name','entity_id','pos_value_thousand_sar','transaction_count','next_value','next_ratio'}
...
def main():
    d,F=prepare(); chosen,target_diag=choose_target(d); decline=float(chosen['threshold']); d['target']=(d.next_ratio<(1-decline)).astype(int)
    train=d[d.week_start<'2024-01-01']; val=d[(d.week_start>='2024-01-07')&(d.week_start<'2025-01-01')]; test=d[d.week_start>='2025-01-05']
    print(json.dumps({'prepared_rows':len(d),'entities':int(d.entity_id.nunique()),'cities':int((d.entity_type=='City').groupby(d.entity_name).any().sum()),'sectors':int((d.entity_type=='Sector').groupby(d.entity_name).any().sum()),'train':len(train),'validation':len(val),'test':len(test),'features':len(F)},indent=2))
    ...
    for name,spec in models().items():
        q=clone(spec).fit(train[F],train.target); p=q.predict_proba(val[F])[:,1]; best=None
        ...
        selection[name]={'probability_threshold':best[5],'validation_metrics':best[6],'score':best[0]}
